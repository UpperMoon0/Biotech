...
public class FermenterScreen extends AbstractContainerScreen<FermenterMenu> {
    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(Biotech.MOD_ID, "textures/gui/" + MachineRegistries.FERMENTER.id() + ".png");
    private static final int MAX_PROGRESS_WIDTH = 20;
    private static final int PROGRESS_HEIGHT = 16;

    public FermenterScreen(FermenterMenu menu, Inventory inventory, Component component) { super(menu, inventory, component); }
    ...
    protected void renderBg(@NotNull GuiGraphics g, float partialTick, int mouseX, int mouseY) {
        ...
        g.blit(TEXTURE, leftPos + 108, topPos + 50, 212, 0, getProgressWidth() + 1, PROGRESS_HEIGHT);
        IngredientItem[] ingredients = menu.getRecipe().getIngredientItems();
        for (int i = 0; i < ingredients.length; i++) new BiotechItemRenderer(16, 16).render(g.pose(), leftPos + 38 + (i % 3) * 18, topPos + 42 + (i / 3) * 18, ingredients[i].getItemStack());
        OutputItem output = menu.getRecipe().getOutputItems()[0];
        new BiotechItemRenderer(30, 30).render(g.pose(), leftPos + 150, topPos + 51, output.getItemStack());
