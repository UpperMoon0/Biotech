...
public class SlaughterhouseScreen extends AbstractContainerScreen<SlaughterhouseMenu> {
    ...
    protected void renderLabels(@NotNull GuiGraphics g, int mouseX, int mouseY) {
        ...
        if (menu.getIsOperating()) {
            ...
            for (int i = 0; i < outputs.length; i++) {
                ...
                g.drawString(font, outputs[i].getItemStack().getCount() + chance, 143, 29 + 18 * i, 0xFFFFFF);
            }
        }
    ...
    protected void renderBg(@NotNull GuiGraphics g, float partialTick, int mouseX, int mouseY) {
        ...
        g.blit(TEXTURE, leftPos + 86, topPos + 26, 212, 0, getProgressWidth() + 1, PROGRESS_HEIGHT);
        new BiotechItemRenderer(48,48).render(g.pose(), leftPos + 46, topPos + 35, menu.getRecipe().getIngredientItems()[0].getItemStack());
        OutputItem[] outputs = menu.getRecipe().getOutputItems();
        for (int i = 0; i < outputs.length; i++) new BiotechItemRenderer(16,16).render(g.pose(), leftPos + 123, topPos + 24 + 18 * i, outputs[i].getItemStack());
        new BiotechFluidRenderer().renderFluid(g.pose(), leftPos + 31, topPos + 98, 20, 20, menu.getRecipe().getFluidIngredients()[0]);
    }
