...
public class FermenterScreen extends AbstractContainerScreen<FermenterMenu> {
    ...
    protected void renderLabels(@NotNull GuiGraphics g, int mouseX, int mouseY) {
        ...
        if (menu.getIsOperating()) {
            ...
            if (isHovering(31, 131, 20, 20, mouseX, mouseY)) {
                g.renderTooltip(font, List.of(Component.literal(fluidName), Component.literal(recipe.getFluidIngredients()[0].getAmount() + " mB")), Optional.empty(), mouseX - leftPos, mouseY - topPos);
            }
        }
    ...
    protected void renderBg(@NotNull GuiGraphics g, float partialTick, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < ingredientItems.length; i++) {
            int x = 38 + (i % 3) * 18;
            int y = 42 + (i / 3) * 18;
            new BiotechItemRenderer(16, 16).render(g.pose(), leftPos + x, topPos + y, ingredientItems[i].getItemStack());
        }
        OutputItem outputItem = menu.getRecipe().getOutputItems()[0];
        new BiotechItemRenderer(30, 30).render(g.pose(), leftPos + 150, topPos + 51, outputItem.getItemStack());
