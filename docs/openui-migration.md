...
The Mixer and Fermenter retain input grids; Breeding Chamber and Terrestrial Habitat retain animal-to-output diagrams with food and water; Greenhouse retains seed, fertilizer, and centered outputs; Slaughterhouse uses the shared two-column output grid, including its four-output rabbit recipe and output-chance labels. Item counts and output chances remain visible, with item names available on hover. Energy, fluid, and progress tooltips read live synchronized values, including when the pointer is stationary. Unsynchronized capacities and invalid structures do not divide by zero or render stale active recipes.

OpenUI owns component layout, lifecycle, and overlays. Vanilla owns real inventory slots and item tooltips. Screen backgrounds use OpenUI drawing primitives before slot rendering. No texture-backed machine-screen implementation remains. Biotech JEI categories likewise use JEI-native standard slot backgrounds and recipe arrows rather than Biotech-owned recipe background textures; Patchouli continues to own its guide-book presentation.
...
Biotech requires OpenUI MC 0.0.10+ within the 0.0.x compatibility line on clients (`[0.0.10,0.1)`). The coordinated OpenUI source revision is pinned in `gradle.properties`; CI and release jobs publish that exact provider to Maven Local before compiling Biotech.
...
- Six machines, each active, idle, and with an invalid structure (18 PNGs).
  The active Slaughterhouse fixture mirrors the largest production recipe: rabbit with four outputs, including the second output row and the 13% Rabbit Foot chance label.
