...
public class BreedingChamberCategory implements IRecipeCategory<BreedingChamberRecipe> {
    ...
    @Override public void setRecipe(@NotNull IRecipeLayoutBuilder builder,@NotNull BreedingChamberRecipe recipe,@NotNull IFocusGroup focuses){List<Ingredient> items=recipe.getItemIngredients().stream().map(v->Ingredient.of(v.getItemStack())).toList();FluidStack fluid=recipe.getFluidIngredients().get(0);List<Ingredient> outputs=recipe.getItemOutputs().stream().map(v->Ingredient.of(v.getItemStack())).toList();builder.addSlot(RecipeIngredientRole.INPUT,23,1).addIngredients(items.get(0));builder.addSlot(RecipeIngredientRole.INPUT,41,1).addIngredients(items.get(1));builder.addSlot(RecipeIngredientRole.INPUT,32,21).addFluidStack(fluid.getFluid(),fluid.getAmount()).setFluidRenderer(fluid.getAmount(),false,16,16);builder.addSlot(RecipeIngredientRole.OUTPUT,101,11).addIngredients(outputs.get(0));}
